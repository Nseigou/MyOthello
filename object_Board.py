    ...
    def draw(self):
        ...
        for i in range(self.edge):
            ...
            for j in range(self.edge):
                if self.board[i,j] == self.player1.symbol:  #配列の要素が1なら白丸を描写
                    temp.append(self.player1.mark)
                elif self.board[i,j] == self.player2.symbol:  #配列の要素が-1なら黒丸を描写
                    temp.append(self.player2.mark)
                else:                                #0なら＋を描写
                    temp.append("+")
            ...
            for column in range(self.edge):
                if self.board[row][column] == 0:    #もしコマが置いてないなら,シュミレーションをしてひっくり返せるマスなのかを判定
                    ...
                    for dr,dc in direction: #directionの座標要素を順番に取り出す
                        r = row + dr  #縦方向にずらす　
                        c = column + dc  #横方向
                        if (0 <=r<= 7 and 0 <=c<= 7 and self.board[r][c] == -player.symbol):  #相手のコマがその方向に続いてて、最後に自分のコマがある場合Trueを返す
                            while(0 <=r<= 7 and 0 <=c<= 7 and self.board[r][c] == -player.symbol):
                                r += dr 
                                c += dc
                                if (0 <=r<= 7 and 0 <=c<= 7 and self.board[r][c] == player.symbol):
                                    #print("置ける！")
                                    return True
        #print("置けない")
        return False 
